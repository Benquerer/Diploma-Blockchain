...
public class OremoteP2P extends UnicastRemoteObject implements IremoteP2P {
     ...
     */
    @Override
    public byte[] registerUser(String username, byte[] password, boolean isRegist) throws RemoteException {
        try {
            //create the user
            ...
            //return private key
            byte[] secretPrivate = SecurityUtils.encrypt(newUser.getPrivKey().getEncoded(), simkey);
    ...
    public Map<String, byte[]> loginUser(String username, byte[] password) throws RemoteException {
        ...
        if (knownUsers.contains(username)) {
            try {
                ...
                loggedUser.loadPub();
                //decrypt pass that is encrypted with client private key
                byte[] userPass = SecurityUtils.decrypt(password, loggedUser.getPubKey());
