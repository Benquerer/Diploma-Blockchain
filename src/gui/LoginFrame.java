            ...
            try {
                ...
                if (remote.userExists(username)) {
                    ...
                    if (secretServer != null) {
                        ...
                        //registra utilizador no server e recebe a privada
                        byte[] secretPriv = remote.registerUser(username, secretPass, boxIsOrg.isSelected());
                        if (secretPriv != null) {
                            //salvar a privada
                            byte[] privkey = SecurityUtils.decrypt(secretPriv, simServer);
                            PrivateKey key = SecurityUtils.getPrivateKey(privkey);
                            SecurityUtils.saveKey(key, username + ".privkey");
                            
                            JOptionPane.showMessageDialog(rootPane, "User created with success!!", "User creation!", JOptionPane.INFORMATION_MESSAGE);
                        } else {

                            JOptionPane.showMessageDialog(rootPane, "User created with success!!", "User creation!", JOptionPane.ERROR_MESSAGE);
                        }
                    }
                    txtRegUsername.setText("");
                    pwdfRegPass.setText("");
    ...
    private void btnLoginActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnLoginActionPerformed
        try {
            ...
            if (secretServer != null) {
                ...
                Key simServer = SecurityUtils.getAESKey(simkeyServer);
                //encriptar pwd com a simetrica do server
                byte[] secretPass = SecurityUtils.encrypt(pwd.getBytes(), SecurityUtils.getPrivateKey(Files.readAllBytes(Path.of(username + ".privkey"))));
